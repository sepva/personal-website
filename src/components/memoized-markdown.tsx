...
const markdownComponents: Components = {
  ...
  h3: ({ children }) => (
    <h3 className="text-lg font-bold mb-3 mt-4 first:mt-0">{children}</h3>
  ),

  // Style lists with proper spacing and bullets/numbers
  ul: ({ children }) => (
    <ul className="mb-4 space-y-2 pl-6 list-disc">{children}</ul>
  ),
  ol: ({ children }) => (
    <ol className="mb-4 space-y-2 pl-6 list-decimal">{children}</ol>
  ),
