...
export function ChatBubble({ type, children }: ChatBubbleProps) {
  const styles = {
    ...
    system: 'bg-[#252831] text-[#D1D5DB] border border-[#2F323D] italic text-center'
  };
  ...
  return (
    <div className={`${widthStyles[type]} ${type === 'user' ? 'ml-auto' : ''}`}>
      <div className={`${styles[type]} rounded-[18px] px-[20px] py-[16px] shadow-[0_4px_8px_rgba(0,0,0,0.04)]`}>
        {children}
      </div>
    </div>
  );
