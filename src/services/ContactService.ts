...
export class ContactService {
   ...
   */
  private async checkGlobalRateLimit(globalLimit: number): Promise<{ exceeded: boolean; count: number }> {
    const oneHourAgo = Date.now() - (60 * 60 * 1000);
   ...
   */
  private checkSessionRateLimit(sessionLimit: number): { exceeded: boolean; minutesUntilAvailable?: number } {
   ...
   */
  private async checkEmailRateLimit(email: string, emailLimit: number): Promise<{ exceeded: boolean; count: number }> {
    const oneHourAgo = Date.now() - (60 * 60 * 1000);
    const oneHourAgoISO = new Date(oneHourAgo).toISOString();
  ...
  async saveContactMessage(
    ...
    try {
      ...
      const globalLimit = Number.parseInt(env?.RATE_LIMIT_GLOBAL_PER_HOUR || "100", 10);
      const sessionLimit = Number.parseInt(env?.RATE_LIMIT_SESSION_PER_HOUR || "3", 10);
      const emailLimit = Number.parseInt(env?.RATE_LIMIT_EMAIL_PER_HOUR || "3", 10);
      ...
      if (globalCheck.exceeded) {
        console.warn(`Global rate limit reached: ${globalCheck.count} submissions in last hour`);
        return {
          success: false,
          error: "Service is currently experiencing high traffic. Please try again later."
        };
      }
      console.log(`Global submissions in last hour: ${globalCheck.count}/${globalLimit}`);
